...
def parse_beeline_log(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            # Stop parsing if the line contains "Completed executing command"
            if "Completed executing command" in line:
                break

            # Detect the start of sections and update current_section and current_metric_group
            current_section, current_metric_group = update_section(line, current_section, current_metric_group, detailed_metrics)
...
def update_section(line, current_section, current_metric_group, detailed_metrics):
    ...
    elif line.endswith(':') and not line.startswith("INFO  :"):
        current_metric_group = line.strip().rstrip(':')
...
def parse_detailed_metrics(line, current_metric_group, detailed_metrics):
    ...
    if ':' in line and current_metric_group:
        ...
        if len(key_value) == 2:
            ...
            try:
                ...
                pass  # Keep value as a string if it can't be converted to int
